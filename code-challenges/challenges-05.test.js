...
const toLastNames = people => {
  // Solution code here...
  return people.map(ppl => `${ppl.firstName} ${ppl.lastName}`);
};
...
const addValues = (arr) => {
  let total = arr.reduce((answerSoFar, value) => {
...
Write a function named addPurchases that, given an  as input, uses reduce to find the total amount purchased. Each object contains the keys `itarray of objectsem` and `purchasePrice` like the example.
...
const addPurchases = (arr) => {
  let total = arr.reduce((answerSoFar.purchasePrice, value.purchasePrice) => {
    answerSoFar.purchasePrice = answerSoFar.purchasePrice + value;
    return answerSoFar;
  },0);
  return total;
  // Solution code here...

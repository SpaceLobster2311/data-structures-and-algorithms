...
const count = (target, input) => {
  ...
  // // Solution code here...
  // );
  // with help from Kassie, Garfield, Kevin
};
/*------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const totalSum = (input) => {
  let answer = input.map(arr => arr.reduce((acc, elem) => acc + elem,0));

  return answer.reduce(function(a, b) { return a + b; }, 0);
  // Solution code here...
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(arr => arr.filter(x => (typeof x) === 'number' && (x % 5 === 0)).map(x => Math.pow(2,x)));
};
